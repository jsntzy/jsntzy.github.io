 ...
 //向GitHub上传图片
      ...
      async function submitToGitHub(content, picdata, fileName,callback) {
        ...
        const settings = {
            ...
            token: 'eD7U1sXDp2kykzrz4ytrkpODD2S6FC2e3OrE',
            branch: 'main',
    ...
    reader.readAsDataURL(imageFile);
        ...
        const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzWib_CTvDDfGg8FBxtBc7loPkn9JtjvDwNnDMi7Z7x6FE7lNGXXPu8EJDFRjmtmHCgyQ/exec';
      ...
      function hideModal(modalOverlay) {
       ...
       //点赞
        async function likeCard(_id){
            try {
                  var data=await postData('addlike',{id:_id});   
									addLike_comm(_id,0);
            } catch (error) {
       ...
       async function PostComment(_id,commentConent){
 						try {
                  var data=await postData('comment',{rid:_id,comment:commentConent,name:author},function(data){
